...
1. **Skill-trigger validation tooling** *(replaces what was previously framed as "ambient nudge hook")*. Claude Code already routes prompts to skills via the `description:` field — building a meta-routing hook on top duplicates that work and probably degrades it. The real leverage is making it easy for skill authors to write descriptions that actually trigger when they should. Concrete shape:
    - A `triggers:` block (frontmatter or sibling file like `triggers.md`) listing example user prompts the skill should fire on, plus optional negative examples it should NOT fire on.
    - Lint on description quality (already partially in `docs/pr-checklist.md`): front-loaded use case, action verb instead of "helps with", trigger words a real user would say.
    - Optional: a model-based eval harness (gated by env var since it costs API credits) that runs each declared trigger prompt through Claude and confirms the right skill is invoked. The Anthropic skill-creator skill already does something like this — worth borrowing the pattern rather than inventing.
    - Output: skill authors get fast feedback on "does my description actually catch what I want?" before users discover that it doesn't.
