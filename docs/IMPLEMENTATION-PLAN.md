...
**Implication:** Don't build a new marketplace runtime. Lean on Claude Code's plugin marketplace as the substrate. Classroom v1 is a *convention layer* + a *Guide skill* + an *install template* on top of it. Net-new code is small.

### Layer 2 — Three repos, one convention

```
company-classroom/                  # Tier 1: Central (immutable to users)
...
2. **First-run prompt** — `SessionStart` hook welcomes the user and tells them to type `/classroom`.
...
| 6 | First-run SessionStart hook (written by install.sh) | `~/.claude/classroom-first-run.sh` |
...
2. Open Claude Code → SessionStart hook fires → user types `/classroom` → Guide asks team/role → recommends 3 skills.
...
5. `/classroom extend [skill_name]` → Guide generates a local extension → parent still updates from central without touching the extension.
6. Team lead runs `/classroom curate-path` → PR opens against central → after merge, new team members get that path.
...
> **Principal review incorporated (revision 2).** All five must-fixes folded in inline below: (1) optional checksum verification via `CLASSROOM_TARBALL_SHA256`, (2) destructive-path guard before any `rm -rf`, (3) atomic dir swap to eliminate the read-during-write race with `refresh.sh`, (4) lock file in `refresh.sh` to prevent concurrent execution, (5) portable `( ... & )` subshell instead of `& disown`. Also incorporated: tightened dev-checkout detection (now requires `.git/`), local-edit overwrite warning, refresh-log timestamp for observability. Out-of-scope items from the review (full versioning, schema validation, advanced observability) remain out of scope.

## Context

v1 ships with `install.sh` requiring `git` on the user's machine and cloning the central repo to `~/.claude/classroom/`. Two problems for the non-technical target user:
...
2. **`git clone` is the wrong primitive for a snapshot consumer.** The user never pulls, branches, or commits inside `~/.claude/classroom/`. They just need the files. A tarball download is exactly that, with no preflight dependency.
...
- Replace the three-branch clone block (lines 53–64):
  - Compute tarball URL: default `${CLASSROOM_REPO%.git}/archive/refs/heads/${CLASSROOM_REF}.tar.gz` (GitHub layout). Allow override via `CLASSROOM_TARBALL` env var for non-GitHub hosts.
  - **Dev-checkout detection** (preserves the local-dev and test-sandbox flow): treat as dev checkout *only* if `$CLASSROOM_DIR/.git` exists. (Tightened per principal review — the previous "guide/SKILL.md + marketplace.json present" heuristic could false-positive against a half-extracted tarball.) For the tests/sandbox flow that doesn't have a `.git/`, the test harness will set a new explicit env var `CLASSROOM_USE_INPLACE=1` to force in-place use.
  - **Tarball integrity (optional checksum).** If `CLASSROOM_TARBALL_SHA256` is set, after download run `echo "$CLASSROOM_TARBALL_SHA256  $tarball_file" | shasum -a 256 -c -` and abort on failure. Optional, not required — preserves the default zero-friction path while letting secure environments lock down integrity.
  - Otherwise: download tarball into a temp dir under `$(mktemp -d)`, `tar -xz --strip-components=1` into a staging subdir.
  - **Atomic swap** (per principal review — fixes race with `refresh.sh` reads): move the existing `$CLASSROOM_DIR` aside to `$CLASSROOM_DIR.old`, move staging dir to `$CLASSROOM_DIR`, then `rm -rf` the old. Single `mv` of a sibling dir on the same filesystem is atomic. No window where the Guide can read a half-written cache.
  - **Destructive-path guard** (per principal review): before any `rm -rf` of `$CLASSROOM_DIR.old`, assert `[[ "$CLASSROOM_DIR" == *".claude/classroom"* ]]` and abort otherwise. Cheap insurance against a malformed env var nuking the wrong directory.
  - **Local-edit visibility:** if the swap replaced an existing populated cache that wasn't a dev checkout, print a one-line warning: `Classroom cache refreshed (any local edits in ~/.claude/classroom were overwritten)`. Per principal review — prevents silent surprise.
- Update the usage comment block at the top to drop the git mention. Document `CLASSROOM_TARBALL`, `CLASSROOM_TARBALL_SHA256`, and `CLASSROOM_REF` (which already supports tags — call this out for users who want to pin to a release tag rather than `main`).

**2. `install-dev.sh` — new file, contributor flow**

- Copy of the *current* install.sh git-clone path. Top comment: "For Classroom contributors. Regular users should use install.sh."
...
- Add a new "How Classroom stays current" subsection (see "How updates flow" below).

**4. `tests/run.sh` — extend Layer 4**

The current Layer 4 (around line 304) pre-populates `$SANDBOX_CLASSROOM` with a non-git copy and exercises the "exists but not git" branch. Under the new `install.sh`, this maps cleanly to the new dev-checkout-detection branch (the sandbox copy contains both `guide/SKILL.md` and `.claude-plugin/marketplace.json`), so all 13 existing sandbox tests should pass unchanged. Verify this.

The current Layer 4 sandbox flow (which uses a non-git copy of the repo as `$CLASSROOM_DIR`) will be updated to set the new explicit `CLASSROOM_USE_INPLACE=1` env var to force in-place use, since the dev-checkout heuristic is being tightened to require `.git/`. Existing 13 sandbox tests should still pass after this one-line tweak.

Add new tests (mapped to principal review must-fixes):

- **Tarball install path.** Create a fresh `$SANDBOX2` with no pre-populated `CLASSROOM_DIR`. Build a local tarball: stage `$SANDBOX2/stage/classroom/` with the repo contents, then `tar -czf classroom.tar.gz -C $SANDBOX2/stage classroom`. Run `install.sh` with `CLASSROOM_TARBALL=file://$SANDBOX2/classroom.tar.gz`. Assert `$SANDBOX2/.claude/classroom/guide/SKILL.md` exists and matches source, `settings.json` registered, hook installed.
- **No git on PATH.** Re-run `install.sh` under a stripped PATH (e.g. `PATH=/usr/bin:/bin`) and confirm exit 0.
- **Checksum validation passes** when `CLASSROOM_TARBALL_SHA256` matches the local tarball.
- **Checksum validation fails loudly** when `CLASSROOM_TARBALL_SHA256` is set but wrong — `install.sh` exits non-zero, `$CLASSROOM_DIR` is unchanged.
- **Destructive-path guard** rejects an obviously bad `CLASSROOM_DIR` (e.g. `/tmp/something-else`) — install aborts before deleting anything.
- **Atomic swap.** After a successful tarball install, assert no `$CLASSROOM_DIR.old` or `$CLASSROOM_DIR.new` siblings linger.
- **Update freshness.** Run tarball install once, delete a file from `$SANDBOX2/.claude/classroom/plugins/...`, re-run `install.sh`, confirm the file is restored. Add a stray file inside the cache, re-run, confirm it's gone (atomic swap inherently delivers `--delete` semantics — the entire dir is replaced).
...
- New file `refresh.sh`:
  - **Lock file** (per principal review — prevents concurrent refreshes from racing each other): `LOCK_FILE="$HOME/.claude/classroom-refresh.lock"`. If exists, exit 0 silently. Otherwise `touch "$LOCK_FILE"; trap 'rm -f "$LOCK_FILE"' EXIT`.
  - Reads `$HOME/.claude/classroom-last-refresh` marker. If younger than 24h, exit 0 silently (after releasing lock).
  ...
  - On success, `touch` the marker and log timestamp to `$HOME/.claude/classroom-refresh.log` (per principal review #9 — minimal observability so silent staleness is debuggable).
  - On network or extract failure, log to `$HOME/.claude/classroom-refresh.log` and exit 0 — never break the user's session.
  - Skips itself entirely if `$CLASSROOM_DIR/.git` exists — contributors update via `git pull`.
- Modify `install.sh` to: (a) drop `refresh.sh` next to the existing first-run hook, and (b) extend the first-run hook script to invoke `refresh.sh` in a portable subshell (per principal review — `& disown` is shell-dependent): `( "$HOME/.claude/classroom-refresh.sh" >/dev/null 2>&1 & )`. The parent hook returns immediately; the refresh runs detached without relying on `disown`. The hook stays a single command in `settings.json`.
- Tests: add Layer 4 tests asserting (i) `refresh.sh` exists post-install and is executable, (ii) running `refresh.sh` twice in succession only does work once (marker honored), (iii) running `refresh.sh` with a corrupted/unreachable tarball URL still exits 0 and writes to the log file, (iv) `refresh.sh` is a no-op when `$CLASSROOM_DIR/.git` exists, (v) **lock file blocks concurrent runs**: create the lock file, run refresh, assert it exits 0 without doing work; then remove the lock and assert it can run again, (vi) **atomic swap leaves no partial state**: simulate a refresh that aborts mid-extract (e.g. corrupt tarball), assert `$CLASSROOM_DIR` is unchanged afterward (no `.partial` or half-extracted dirs).
- Override: power users can set `CLASSROOM_REFRESH_INTERVAL=0` to force every-session refresh, or `CLASSROOM_REFRESH_INTERVAL=-1` to disable. Document in README.
...
- Optionally add a `/classroom update` subcommand that re-invokes `install.sh` after confirmation (see "How updates flow" #3).
...
1. **Reference cache** (`~/.claude/classroom/`): refreshed by `refresh.sh` on SessionStart, at most once per 24h, in the background, never blocking. This is what feeds the Guide's `recommend` and `extend` flows — when central adds a new plugin or updates a path, users see it within a day without re-running `install.sh`.
2. **Installed plugins** (the things users actually run): updated by Claude Code's existing per-marketplace `autoUpdate` toggle on `extraKnownMarketplaces`. `install.sh` should set this to `true` by default for `classroom-reference`. Existing primitive — we're just opting in.
3. **The Guide skill itself** (`~/.claude/skills/classroom/SKILL.md`): refreshed only by re-running `install.sh`, because changing the Guide is a privileged operation we don't want happening silently. Add a Guide subcommand `/classroom update` that re-invokes `install.sh` after confirmation. This honors the "always confirm before action" rule for the one update channel that touches a skill the user is actively running.

Document all three in `README.md` under a new "How Classroom stays current" subsection.
...
2. **Manual sandbox without git on PATH:**
   ...
   ```
   Confirm no error, `$SANDBOX/.claude/skills/classroom/SKILL.md` exists, `$SANDBOX/.claude/classroom/.claude-plugin/marketplace.json` exists.
3. **Idempotency:** run `install.sh` twice in the same sandbox; second run succeeds, hooks not duplicated, files refreshed.
4. **Refresh:** force `CLASSROOM_REFRESH_INTERVAL=0`, run the SessionStart hook, confirm `refresh.sh` ran, then with default interval confirm the second invocation was a no-op.
5. **Dev flow:** `bash install-dev.sh` against a sandbox; `~/.claude/classroom/.git` exists and `git -C ~/.claude/classroom status` works.
6. **Ownership:** `/classroom owners` lists every plugin with owner + contact; `tests/run.sh` enforces every plugin has an owner and a CODEOWNERS line.
7. **End-to-end Guide flow** (manual): post-install sandbox, open Claude Code, `/classroom`, walk `recommend` → `install` for one plugin. Confirm the Guide reads `paths/sales-ae.md` from the tarball-installed dir.
...
- [ ] Is `CLASSROOM_TARBALL` override sufficient for non-GitHub hosts (GitLab, Bitbucket, internal Gitea)? Should we also support `CLASSROOM_TARBALL_HEADER` for auth?
- [ ] Is `rsync --delete` aggressive enough? Will it ever wipe a user's local edits inside `~/.claude/classroom/`? (Answer should be: yes it will, and that's correct because the cache is a snapshot — but document it.)
...
- [ ] Should `/classroom update` ALSO invalidate the 24h refresh marker so the next session pulls fresh?
